...
public class Magnet implements CustomEffect {
    ...
    public void onTick(Player player, ItemStack item, int level) {
        int per = Math.max(1, com.specialitems.util.Configs.cfg.getInt("effects.magnet.range-per-level", 3));
        int max = Math.max(per, com.specialitems.util.Configs.cfg.getInt("effects.magnet.max-range", 10));
