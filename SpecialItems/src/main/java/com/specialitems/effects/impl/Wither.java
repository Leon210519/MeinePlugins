...
public class Wither implements CustomEffect {
    ...
    public void onEntityDamage(Player player, ItemStack weapon, EntityDamageByEntityEvent e, int level) {
        ...
        double chance = com.specialitems.util.Configs.cfg.getDouble("effects.wither.chance", 0.25);
        if (ThreadLocalRandom.current().nextDouble() <= chance) {
            int dur = com.specialitems.util.Configs.cfg.getInt("effects.wither.duration-ticks-per-level", 40) * level;
            int amp = Math.max(0, com.specialitems.util.Configs.cfg.getInt("effects.wither.amplifier", 0));
