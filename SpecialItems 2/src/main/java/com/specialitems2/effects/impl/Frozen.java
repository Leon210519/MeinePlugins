...
public class Frozen implements CustomEffect {
    ...
    public void onEntityDamage(Player player, ItemStack weapon, EntityDamageByEntityEvent e, int level) {
        ...
        double chance = com.specialitems2.util.Configs.cfg.getDouble("effects.frozen.chance", 0.25);
        if (ThreadLocalRandom.current().nextDouble() <= chance) {
            int dur = com.specialitems2.util.Configs.cfg.getInt("effects.frozen.duration-ticks-per-level", 40) * level;
            int amp = Math.max(0, com.specialitems2.util.Configs.cfg.getInt("effects.frozen.amplifier", 0));
