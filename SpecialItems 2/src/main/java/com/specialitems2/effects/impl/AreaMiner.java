...
public class AreaMiner implements CustomEffect {
    ...
    public void onBlockBreak(Player player, ItemStack tool, BlockBreakEvent e, int level) {
        int radius = Math.min(com.specialitems2.util.Configs.cfg.getInt("effects.areaminer.radius", 1) + (level-1), 3);
        int max = com.specialitems2.util.Configs.cfg.getInt("general.max-broken-blocks", 128);
        int broken = 0;
