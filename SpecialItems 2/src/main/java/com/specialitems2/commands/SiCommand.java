...
import com.specialitems2.leveling.LevelMath;
import com.specialitems2.leveling.LevelOverviewGUI;
import com.specialitems2.leveling.ToolClass;
import com.specialitems2.util.Configs;
import com.specialitems2.util.ItemUtil;
import com.specialitems2.util.Tagger;
...
public class SiCommand implements CommandExecutor {
    ...
    private static void sendHelp(CommandSender sender) {
        sender.sendMessage(ChatColor.GOLD + "SpecialItems2 " + ChatColor.GRAY + "— Commands:");
        sender.sendMessage(ChatColor.YELLOW + "/si2 gui" + ChatColor.GRAY + " — Open templates GUI (admin)");
        sender.sendMessage(ChatColor.YELLOW + "/si2 give " + ChatColor.WHITE + "<player> <template>" + ChatColor.GRAY + " — Give a template (admin)");
        sender.sendMessage(ChatColor.YELLOW + "/si2 list " + ChatColor.WHITE + "<effects|templates>" + ChatColor.GRAY + " — Show lists");
        sender.sendMessage(ChatColor.YELLOW + "/si2 reload" + ChatColor.GRAY + " — Reload configuration (admin)");
        sender.sendMessage(ChatColor.YELLOW + "/si2 levels" + ChatColor.GRAY + " — Open level overview");
        sender.sendMessage(ChatColor.YELLOW + "/si2 inspect" + ChatColor.GRAY + " — Inspect held item");
        sender.sendMessage(ChatColor.YELLOW + "/si2 publicinspect" + ChatColor.GRAY + " — Broadcast held item");
        sender.sendMessage(ChatColor.YELLOW + "/si2 retag " + ChatColor.WHITE + "[id]" + ChatColor.GRAY + " — Tag held item as Special (admin)");
    }
    ...
    private static boolean requireAdmin(CommandSender sender) {
        if (sender.hasPermission("specialitems2.admin")) {
            return true;
        }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (sub) {
            ...
            case "give" -> {
                ...
                if (args.length < 3) {
                    sender.sendMessage(ChatColor.RED + "Usage: /si2 give <player> <templateId>");
                    return true;
                }
            ...
            case "reload" -> {
                if (!requireAdmin(sender)) return true;
                Configs.load(SpecialItems2Plugin.getInstance());
                java.util.List<com.specialitems2.util.TemplateItems.TemplateItem> list = TemplateItems.loadAll();
                for (com.specialitems2.util.TemplateItems.TemplateItem t : list) CustomModelDataUtil.normalize(t.stack());
                sender.sendMessage(ChatColor.GREEN + "SpecialItems2 configuration reloaded.");
                return true;
            }
            case "retag" -> {
                if (!requirePlayer(sender) || !requireAdmin(sender)) return true;
                ...
                Tagger.tagAsSpecial(SpecialItems2Plugin.getInstance(), held, id);
            ...
            case "list" -> {
                if (args.length < 2) {
                    sender.sendMessage(ChatColor.RED + "Usage: /si2 list <effects|templates>");
                    return true;
                }
