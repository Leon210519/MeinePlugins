...
import com.specialitems2.leveling.Keys;
import com.specialitems2.cmd.CmdRegistry;
import com.specialitems2.util.CustomModelDataUtil;
import com.specialitems2.leveling.Rarity;
import com.specialitems2.leveling.RarityUtil;
...
public final class TemplateItems {
    ...
    public static TemplateItem buildFrom(String id, ConfigurationSection t) {
        ...
        try {
            Tagger.tagAsSpecial(SpecialItems2Plugin.getInstance(), it, id);
        ...
        if (rar != null) {
            try {
                ...
                RarityUtil.set(it, new Keys(SpecialItems2Plugin.getInstance()), r);
            } catch (Throwable ignored) {}
        }
    ...
    public static List<TemplateItem> getByRarity(Rarity rarity) {
        ...
        for (TemplateItem t : loadAll()) {
            Rarity r = RarityUtil.get(t.stack(), new Keys(SpecialItems2Plugin.getInstance()));
    ...
    public static boolean applyTemplateMeta(ItemStack item) {
        ...
        Integer allowed = CmdRegistry.clamp(tmeta.getCustomModelData());
        meta.setCustomModelData(allowed);
        item.setItemMeta(meta);
        CustomModelDataUtil.normalize(item);
        return true;
    }
}